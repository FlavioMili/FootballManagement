...
The human author is 100% responsible for the code they submit. AI is a tool, not an excuse for poor architecture, memory leaks, or unreadable code.

* **You** must understand every single line of code the AI generates before you commit it.
* **You** are responsible for ensuring the AI's code adheres to our `clang-format` and `clang-tidy` rules.
...
1. **Be rigorous:** AI can sometimes produce code that looks correct but has subtle logic flaws or memory safety issues in C++. Look closely for edge cases.
...
2. **Leave a trace:** You must summarize your steps, decisions, and why you made them. Always update or create a walkthrough artifact.
