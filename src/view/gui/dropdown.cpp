...
bool Dropdown::handleEvent(const SDL_Event& event) {
  if (event.type == SDL_EVENT_MOUSE_MOTION) {
    ...
    if (SDL_PointInRectFloat(&mouse_point, &mainRect)) {
      isOpen = !isOpen;
      return isOpen; // Return true if opened
    } else {
      if (isOpen) {
        for (size_t i = 0; i < optionRects.size(); ++i) {
          if (SDL_PointInRectFloat(&mouse_point, &optionRects[i])) {
      ...
      isOpen = false; // Close if clicked outside
    }
  }
  return false;
}
  ...
  if (isOpen) {
    for (size_t i = 0; i < optionRects.size(); ++i) {
      if (i == (size_t)hoveredIndex) {
        SDL_SetRenderDrawColor(renderer, style.hoverColor.r, style.hoverColor.g, style.hoverColor.b, style.hoverColor.a);
      } else {
        SDL_SetRenderDrawColor(renderer, style.backgroundColor.r, style.backgroundColor.g, style.backgroundColor.b, style.backgroundColor.a);
      }
...
void Dropdown::update(float deltaTime) {
  // Not used for now, but here for future animations etc.
  (void)deltaTime;
}
...
void Dropdown::createTextures() {
  ...
  setSelectedIndex(selectedIndex); // Create texture for the initially selected item
}
...
void Dropdown::renderArrow(const SDL_FRect& buttonRect) const {
  ...
  SDL_FColor color = { style.arrowColor.r / 255.0f, style.arrowColor.g / 255.0f, style.arrowColor.b / 255.0f, style.arrowColor.a / 255.0f };
