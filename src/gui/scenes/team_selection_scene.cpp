...
void TeamSelectionScene::render()
{
  ...
  ImGui::BeginChild("LeaguesRegion", ImVec2(0, LEAGUES_REGION_HEIGHT), true, ImGuiWindowFlags_HorizontalScrollbar);
  for (const auto& league_ref : available_leagues) {
    ...
    if (ImGui::Button(league.getName().c_str(), ImVec2(GUIConstants::MENU_BUTTON_WIDTH, GUIConstants::MENU_BUTTON_HEIGHT))) {
  ...
  if (selected_league_id) {
    ...
    if (ImGui::BeginTable("TeamsTable", TEAM_COLUMNS)) {
      for (size_t i = 0; i < available_teams.size(); i++) {
        ...
        if (ImGui::Button(team.getName().c_str(), ImVec2(-FLT_MIN, GUIConstants::MENU_BUTTON_HEIGHT))) {
