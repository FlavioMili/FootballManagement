...
DataGenerator::DataGenerator(std::shared_ptr<Database> db_ptr,
                             const nlohmann::json &stats_config,
                             const std::vector<std::string> &league_names_v,
                             const std::vector<std::string> &team_names_v)
  : db(db_ptr), stats_config(stats_config), league_names(league_names_v),
  team_names(team_names_v) {
...
void DataGenerator::generateTeams(int league_id,
...
void DataGenerator::generatePlayers(int team_id, const std::string &role,
                                    int count) {
  int age;
  for (int i = 0; i < count; ++i) {
    std::string name =
      first_names[static_cast<size_t>(rand()) % first_names.size()] + " " +
      last_names[static_cast<size_t>(rand()) % last_names.size()];
    age = 16 + rand() % 15;
    ...
    for (const auto &stat_name : stats_config["possible_stats"]) {
      stats[stat_name] =
        static_cast<float>(MIN_STAT_VAL +
                           (rand() % (MAX_STAT_VAL - MIN_STAT_VAL))) +
        static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
      if (stats[stat_name] > MAX_STAT_VAL)
        stats[stat_name] = MAX_STAT_VAL;
    }
...
void DataGenerator::generateAll() {
  ...
  for (const auto &league : leagues) {
    ...
    for (size_t i = 0;
    i < TEAMS_PER_LEAGUE && i < current_league_team_names.size(); ++i) {
